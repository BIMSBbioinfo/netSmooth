...
#' @param makeConsensusProportion    proportion of times samples need to be
#'                                 co-clustered for co-clustering step
#' @param makeConsensusMinSize    minimum cluster size
...
clusterExperimentWorkflow <- function(se,
  ...
  ce <- makeConsensus(ce, proportion=makeConsensusProportion,
                      minSize=makeConsensusMinSize,
                      clusterLabel="makeConsensus",
                      whichClusters = 'all')
  if(runMergeClusters) {
    # Make a dendrogram
    ce <- makeDendrogram(ce, reduceMethod="var", nDims=500,
                         whichCluster="makeConsensus")
